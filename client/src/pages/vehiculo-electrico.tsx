...
import { electricVehicles } from '@/data/electric-vehicles';

export default function VehiculoElectricoPage() {
  ...
  const vehicle = slug ? electricVehicles[slug] : null;
  ...
  return (
    <main role="main" className="bg-white">
      <SEOHead
        title={`${vehicle.brand} ${vehicle.name} - Vehículo Eléctrico`}
        ...
        canonical={`/vehiculos/electricos/${slug}`}
        jsonLd={[
          buildVehicleJsonLd({ name: `${vehicle.brand} ${vehicle.name}`, brand: vehicle.brand, imageUrl: vehicle.image, description: vehicle.tagline, slug: slug!, type: "electrico" }),
          buildBreadcrumbJsonLd([
            { name: "Inicio", url: "https://electricos.grupoavisa.com/" },
            { name: "Eléctricos", url: "https://electricos.grupoavisa.com/promociones-electricos" },
            { name: `${vehicle.brand} ${vehicle.name}`, url: `https://electricos.grupoavisa.com/vehiculos/electricos/${slug}` }
          ])
        ]}
      />
      <section className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 py-16 overflow-hidden" aria-label="Información del vehículo eléctrico">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-block px-4 py-2 bg-green-500/20 backdrop-blur-sm text-green-400 text-sm font-bold rounded-full mb-6 border border-green-500/30">
                <i className="ri-flashlight-line mr-2" aria-hidden="true"></i>100% Eléctrico · Etiqueta CERO
              </div>
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-10">
            <div>
              <label className="block text-sm font-bold text-slate-900 mb-4">
                <i className="ri-speed-line mr-2 text-green-500" aria-hidden="true"></i>Kilometraje anual
              </label>
            ...
            <div>
              <label className="block text-sm font-bold text-slate-900 mb-4">
                <i className="ri-calendar-line mr-2 text-green-500" aria-hidden="true"></i>Duración del contrato
              </label>
              <div className="space-y-3">
                {['36', '48', '60'].map((months) => (
                  <button
              ...
              <ul className="space-y-2 text-sm text-slate-600">
                {items.map((item: string, idx: number) => (
                  <li key={idx} className="flex items-start gap-2">
                    <i className="ri-check-line text-green-500 mt-0.5" aria-hidden="true"></i>
                    {item}
                  </li>
                ))}
              </ul>
      ...
      <section className="bg-gradient-to-br from-green-600 to-green-700 py-16" aria-label="Ventajas del modelo">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-12">
